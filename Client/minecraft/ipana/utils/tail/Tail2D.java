...
public class Tail2D {
    ...
    private double[] renderPosition = new double[2];

    public void render(boolean depth) {
        double x = renderPosition[0];
        double y = renderPosition[1];

        if (System.currentTimeMillis()-ms > boneMs) {
            bones.add(new Bone(x, y,boneTicks));
        ...
        for (Bone bone : bones) {
            ...
            GL11.glVertex2d(bone.x - x, bone.y - y);
        }
    ...
    class Bone {
        ...
        public Bone(double x, double y, int time) {
            this.x = x;
            this.y = y;
            this.time = time;
        }
