...
public class NCP311 extends PhaseMode {

    public NCP311(Phase parent) {
        super("NCP 3.11", parent);
    }
    private int fast;
    private boolean goToNextBlock;
    private boolean shiftIntoBlock;
    ...
    public void onMove(EventMoving event) {
        ...
        if (!goToNextBlock) {
            if (isInBlock) {
                ...
                if (isInBox(bb) && !mc.gameSettings.keyBindSneak.pressed && !mc.gameSettings.keyBindJump.pressed) {
                    double base = baseSpeed311(event.getX(), event.getZ());
                    double threshold = base / 3.3;
                    double speed = base;
                    if (getParent().fastMove.getValue() && moveTicks == 1 && mc.thePlayer.onGround) {
                        speed = base * 2 + threshold;
                    }
                    double[] c = PlayerUtils.calculate(speed);
                    ...
                    if ((pos.getX() != offsetPos.getX() || pos.getZ() != offsetPos.getZ())) {
                        speed = calcDistToNextBlock(speed);
                        if (speed > base && speed - base <= threshold) {
                            speed = base;
                        }
                        c = PlayerUtils.calculate(speed);
                        goToNextBlock = true;
                    }
                    if (speed > base) {
                        boosted = true;
            ...
            if (!isInBox(bb)) {
                ...
                if (collision != null && diffBetweenCollision(collision) <= 0.0625) {
                    ...
                    if (getBBFromXYZ(mc.thePlayer.posX, mc.thePlayer.posY - 1.0001, mc.thePlayer.posZ) != null) {
                        ...
                        PlayerUtils.cancelFlag(colX, mc.thePlayer.posY, colZ, true);
                    } else {
                ...
                if (downwardsCoolDown <= 0 && getBBFromXYZ(mc.thePlayer.posX, mc.thePlayer.posY-1, mc.thePlayer.posZ) != null) {
                    ...
                    sendOffset(0, -5E-4, 0, true);
                    sendOffset(0, -0.0626, 0, true);
