    ...
    public static void drawDankLitGoalBox(Entity player, Goal goal, float partialTicks, Color color) {
        ...
        if (goal instanceof IGoalRenderPos) {
            ...
            if (settings.renderGoalXZBeacon.value) {
                ...
                Helper.mc.getTextureManager().bindTexture(TileEntityBeaconRenderer.beaconBeam);

                if (settings.renderGoalIgnoreDepth.value) {
                    GlStateManager.disableDepth();
                }

                TileEntityBeaconRenderer.renderBeamSegment(goalPos.getX() - renderPosX, -renderPosY, goalPos.getZ() - renderPosZ, settings.renderGoalAnimated.value ? partialTicks : 0, 1.0, settings.renderGoalAnimated.value ? player.worldObj.getTotalWorldTime() : 0, 0, 256, color.getColorComponents(null), 0.2, 1);
