    ...
    private boolean scanChunkInto(int chunkX, int chunkZ, Chunk chunk, BlockOptionalMetaLookup filter, Collection<BlockPos> result, int max, int yLevelThreshold, int playerY, int[] coordinateIterationOrder) {
        ...
        for (int yIndex = 0; yIndex < 16; yIndex++) {
            ...
            for (int i = 0; i < imax; i++) {
                var container = Block.BLOCK_STATE_IDS.getByValue(extendedblockstorage.getDataArray()[i]);
                IBlockState state = container == null ? Blocks.air.getDefaultState() : container;
                if (filter.has(state)) {
                    int y = yReal | ((i >> 8) & 15);
                    if (result.size() >= max) {
