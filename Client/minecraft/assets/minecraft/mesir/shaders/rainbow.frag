...
void main() {
    ...
    float sussana = passPosition.y;
    if(direction == 1) {
    	sussana = passPosition.x;
    } 
    if(direction == 2) {
    	sussana = passPosition.x * passPosition.y;
    } 
    if(direction == 3) {
    	sussana = -passPosition.x;
    }
    ...
    if(direction == 5) {
    	sussana = -passPosition.y;
    } 
    float hue = mod(hsv.x + amount + (sussana + yCount)*offset, 1.0);
    float sat = 0.5;
    float val = 1.0;
