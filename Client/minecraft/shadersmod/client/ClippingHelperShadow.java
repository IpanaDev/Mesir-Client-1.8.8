...
public class ClippingHelperShadow extends ClippingHelper
{
    ...
    public boolean isBoxInFrustum(double minX, double minY, double minZ, double maxX, double maxY, double maxZ)
    {
        for (int i = 0; i < this.shadowClipPlaneCount; ++i)
        {
            ...
            if (this.dot4(afloat, minX, minY, minZ) <= 0.0D && this.dot4(afloat, maxX, minY, minZ) <= 0.0D && this.dot4(afloat, minX, maxY, minZ) <= 0.0D && this.dot4(afloat, maxX, maxY, minZ) <= 0.0D && this.dot4(afloat, minX, minY, maxZ) <= 0.0D && this.dot4(afloat, maxX, minY, maxZ) <= 0.0D && this.dot4(afloat, minX, maxY, maxZ) <= 0.0D && this.dot4(afloat, maxX, maxY, maxZ) <= 0.0D)
            {
                return false;
            }
        }
