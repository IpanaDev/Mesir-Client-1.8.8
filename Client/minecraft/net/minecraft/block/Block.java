...
public class Block
{
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos normalPos, BlockPos pos, EnumFacing side) {
        /*
        if (Config.isDynamicBlockRender() && Minecraft.getMinecraft().thePlayer.getFaces() != null) {
            PlayerUtils.Face8Direction[] playerFace = Minecraft.getMinecraft().thePlayer.getFaces();
            wasVisible = !((playerFace[0].isOpposite(side.getOpposite()) || playerFace[1].isOpposite(side.getOpposite())) && side != EnumFacing.UP && side != EnumFacing.DOWN);
            if (wasVisible) {
                return false;
            }
        }
        ...
        IBlockState sideState = worldIn.getBlockState(pos);
        boolean opaque = sideState.getBlock().isOpaqueCube();
        if (!opaque) {
            if (this instanceof BlockSnow) {
                IBlockState normalState = worldIn.getBlockState(normalPos);
                if (sideState.getBlock() instanceof BlockSnow && normalState.getBlock() instanceof BlockSnow && normalState.getValue(BlockSnow.LAYERS) <= sideState.getValue(BlockSnow.LAYERS)) {
                    opaque = true;
                }
            } else if ((this instanceof BlockIce && sideState.getBlock() instanceof BlockIce) || (this instanceof BlockPackedIce && sideState.getBlock() instanceof BlockPackedIce)) {
                opaque = true;
            }
        }
        EnumWorldBlockLayer layer = RenderUtils.fixBlockLayer(this, this.getBlockLayer());
        boolean sideCheck = (side == EnumFacing.DOWN && this.minY > 0.0D || (side == EnumFacing.UP && this.maxY < 1.0D || (side == EnumFacing.NORTH && this.minZ > 0.0D || (side == EnumFacing.SOUTH && this.maxZ < 1.0D || (side == EnumFacing.WEST && this.minX > 0.0D || (side == EnumFacing.EAST && this.maxX < 1.0D || !opaque))))));
        return layer == EnumWorldBlockLayer.ORE || sideCheck;
    }
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        EventBoundingBox ebb = new EventBoundingBox(this,pos,axisalignedbb);
        if (collidingEntity == Minecraft.getMinecraft().thePlayer) {
            EventManager.eventSystem.fire(ebb);
        }
        axisalignedbb = ebb.getBoundingBox();
    ...
    public static void registerBlocks()
    {
        ...
        registerBlock(2173, "item_frame", (new BlockItemFrame()).setUnlocalizedName("itemFrame"));
        ...
        for (Block block14 : blockRegistry)
        {
            for (IBlockState iblockstate : block14.getBlockState().getValidStates())
            {
                ...
                //System.out.println(block14.getUnlocalizedName()+" : "+(blockRegistry.getIDForObject(block14) << 4) +" : "+block14.getMetaFromState(iblockstate));
