...
public abstract class BlockLiquid extends Block
{
    ...
    public boolean shouldSideBeRendered(IBlockAccess worldIn, BlockPos normalPos, BlockPos pos, EnumFacing side) {
        ...
        return worldIn.getBlockState(pos).getBlock().getMaterial() != this.blockMaterial && (side == EnumFacing.UP || super.shouldSideBeRendered(worldIn, normalPos, pos, side));
    }
    ...
    public Vec3 getFlowVector(IBlockAccess worldIn, BlockPos pos)
    {
        ...
        if (worldIn.getBlockState(pos).getBlock() instanceof BlockLiquid && worldIn.getBlockState(pos).getValue(LEVEL) >= 8)
        {
    ...
    {
        Vec3 vec3d = getFlowingBlock(p_189544_2_).getFlowVector(p_189544_0_, p_189544_1_);
        return vec3d.xCoord == 0.0D && vec3d.zCoord == 0.0D ? -1000.0F : (float)Math.atan2(vec3d.zCoord, vec3d.xCoord) - ((float)Math.PI / 2F);
    }
