...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO
{
    ...
    private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound p_75820_3_)
    {
        ...
        for (ExtendedBlockStorage extendedblockstorage : aextendedblockstorage)
        {
            if (extendedblockstorage != null)
            {
                ...
                byte[] abyte = new byte[extendedblockstorage.getDataArray().length];

                NibbleArray nibblearray = new NibbleArray();
                NibbleArray nibblearray1 = null;

                for (int i = 0; i < extendedblockstorage.getDataArray().length; ++i) {
                    char c0 = extendedblockstorage.getDataArray()[i];
                    int j = i & 15;
                    int k = i >> 8 & 15;
                    int l = i >> 4 & 15;
    ...
    private Chunk readChunkFromNBT(World worldIn, NBTTagCompound p_75823_2_)
    {
        ...
        for (int l = 0; l < nbttaglist.tagCount(); ++l)
        {
            ...
            char[] containers = new char[abyte.length];

            for (int j1 = 0; j1 < containers.length; ++j1) {
                int k1 = j1 & 15;
                int l1 = j1 >> 8 & 15;
                int i2 = j1 >> 4 & 15;
                int j2 = nibblearray1 != null ? nibblearray1.get(k1, l1, i2) : 0;
                char myHonestData = (char)(j2 << 12 | (abyte[j1] & 255) << 4 | nibblearray.get(k1, l1, i2));
                containers[j1] = myHonestData;
            }
            extendedblockstorage.setDataArray(containers);
