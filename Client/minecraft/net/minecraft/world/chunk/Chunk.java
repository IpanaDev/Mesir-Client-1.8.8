...
public class Chunk
{
    ...
    public IBlockState getBlockState(int x, int y, int z) {
        if (this.worldObj.getWorldType() == WorldType.DEBUG_WORLD) {
            ...
            try {
                ...
                crashreportcategory.addCrashSectionCallable("Location", () -> CrashReportCategory.getCoordinateInfo(x,y,z));
                throw new ReportedException(crashreport);
            }
        }
    }
    ...
    public IBlockState setBlockState(BlockPos pos, IBlockState state)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                } else if (!Modules.BRIGHTNESS.isEnabled()) {
    ...
    public void addEntity(Entity entityIn) {
        ...
        //TODO: better item handling
        if (entityIn instanceof EntityItemFrame itemFrame) {
            //setBlockState(itemFrame.getHangingPosition(), Blocks.itemFrame.getStateFromEntity(itemFrame));
        }
    }
    ...
    public void getEntitiesWithinAABBForEntity(Entity entityIn, AxisAlignedBB aabb, List<Entity> listToFill, Predicate<? super Entity > predicate) {
        int minY = MathHelper.floor_double((aabb.minY - 2.0D) / 16.0D);
        int maxY = MathHelper.floor_double((aabb.maxY + 2.0D) / 16.0D);
        minY = MathHelper.clamp_int(minY, 0, this.entityLists.length - 1);
        maxY = MathHelper.clamp_int(maxY, 0, this.entityLists.length - 1);

        for (int k = minY; k <= maxY; ++k) {
            ClassInheritanceMultiMap<Entity> entities = this.entityLists[k];
            if (!entities.isEmpty()) {
                    ...
                    if (entity != entityIn && entity.getEntityBoundingBox().intersectsWith(aabb)) {
                        ...
                        if (parts != null) {
                            for (Entity part : parts) {
                                if (part != entityIn && part.getEntityBoundingBox().intersectsWith(aabb) && (predicate == null || predicate.apply(part))) {
                                    listToFill.add(part);
                                }
                            }
                        }
    ...
    public boolean isLevelEmpty(int y) {
        ExtendedBlockStorage extendedblockstorage = this.storageArrays[y >> 4];
        return extendedblockstorage == null || extendedblockstorage.isEmpty();
    }
    ...
    public void fillChunk(byte[] p_177439_1_, int p_177439_2_, boolean p_177439_3_) {
        ...
        for (int j = 0; j < this.storageArrays.length; ++j) {
            if ((p_177439_2_ & 1 << j) != 0) {
                ...
                char[] containers = this.storageArrays[j].getDataArray();
                for (int k = 0; k < containers.length; ++k) {
                    if (i + 1 >= p_177439_1_.length) {
                        continue;
                    }
                    char data = (char) ((p_177439_1_[i + 1] & 255) << 8 | p_177439_1_[i] & 255);
                    containers[k] = data;
                    i += 2;
                }
        ...
        if (Modules.WORLD_DUMPER.isEnabled() && Modules.WORLD_DUMPER.listen.getValue()) {
            for (int j1 = 0; j1 < this.storageArrays.length; ++j1) {
                if (this.storageArrays[j1] != null && (p_177439_2_ & 1 << j1) != 0) {
                    for (int y = 0; y < 16; ++y) {
                        for (int z = 0; z < 16; ++z) {
                            for (int x = 0; x < 16; ++x) {
                                IBlockState state = this.storageArrays[j1].get(x, y, z);
                                int xPos = xPosition*16 + x;
                                int yPos = this.storageArrays[j1].getYLocation() + y;
                                int zPos = zPosition*16 + z;
                                Modules.WORLD_DUMPER.addPosState(new BlockPos(xPos, yPos, zPos), state);
                            }
                        }
                    }
                }
    ...
    public BiomeGenBase getBiome(BlockPos pos, WorldChunkManager chunkManager)
    {
        ...
        if (k == 255)
        {
            BiomeGenBase biomegenbase = chunkManager.getBiomeGenerator(pos, plains);
            k = biomegenbase.biomeID;
            this.blockBiomeArray[j << 4 | i] = (byte)(k & 255);
        }

        BiomeGenBase biomegenbase1 = BiomeGenBase.getBiome(k);
        return biomegenbase1 == null ? plains : biomegenbase1;
    }
