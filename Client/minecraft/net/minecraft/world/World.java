...
import static net.minecraft.world.biome.Biomes.plains;
...
public abstract class World implements IBlockAccess
{
    ...
    public final List<EntityLivingBase> livingEntities = Lists.newArrayList();
    ...
    public BiomeGenBase getBiomeGenForCoords(final BlockPos pos)
    {
        ...
        {
            return this.provider.getWorldChunkManager().getBiomeGenerator(pos, plains);
        }
    }
    ...
    public Block getGroundAboveSeaLevel(BlockPos pos)
    {
        ...
        blockpos = new BlockPos(pos.getX(), this.func_181545_F(), pos.getZ());
    ...
    /*
    private Chunk provideChunks(int chunkX, int chunkZ) {
        try {
            return Minecraft.getMinecraft().tickThread.submit(() -> this.chunkProvider.provideChunk(chunkX, chunkZ)).get();
    ...
    {
        return Modules.BRIGHTNESS.isEnabled() ? 1.0f : this.provider.getLightBrightnessTable()[this.getLightFromNeighbors(pos)];
    }
    ...
    public MovingObjectPosition rayTraceBlocksTranslucent(Vec3 vec31, Vec3 vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
            if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
                int i = MathHelper.floor_double(vec32.xCoord);
                int j = MathHelper.floor_double(vec32.yCoord);
                ...
                if (block.getBlockLayer() != EnumWorldBlockLayer.TRANSLUCENT && (!ignoreBlockWithoutBoundingBox || block.getCollisionBoundingBox(this, blockpos, iblockstate) != null) && block.canCollideCheck(iblockstate, stopOnLiquid)) {
                    MovingObjectPosition movingobjectposition = block.collisionRayTrace(this, blockpos, vec31, vec32);
                ...
                while (k1-- >= 0) {
                    ...
                    if ( (!ignoreBlockWithoutBoundingBox || block1.getCollisionBoundingBox(this, blockpos, iblockstate1) != null) && block1.getBlockLayer() != EnumWorldBlockLayer.TRANSLUCENT) {
                        if (block1.canCollideCheck(iblockstate1, stopOnLiquid)) {
                            MovingObjectPosition movingobjectposition1 = block1.collisionRayTrace(this, blockpos, vec31, vec32);
    ...
    /**
     * PlayExec a sound effect. Many many parameters for this function. Not sure what they do, but a classic call is :
    ...
    public void removeEntity(Entity entityIn)
    {
        ...
        {
            if (player.nameTagsList != null) {
                player.nameTagsList.deleteLists();
            }
        ...
        if (entityIn instanceof EntityLivingBase)
        {
            this.livingEntities.remove(entityIn);
        }
    }
    ...
    public void removePlayerEntityDangerously(Entity entityIn)
    {
        ...
        {
            if (player.nameTagsList != null) {
                ((EntityPlayer) entityIn).nameTagsList.deleteLists();
            }
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        return getCollidingBoundingBoxes(entityIn, bb, false);
    }

    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb, boolean entityCheck) {
        ...
        for (int x = minX; x < maxX; ++x) {
            for (int z = minZ; z < maxZ; ++z) {
                if (this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(x, 64, z))) {
        ...
        double d0 = 0.25D;
        AxisAlignedBB boundingBox = bb.expand(d0, d0, d0);
        for (Entity entity : this.loadedEntityList) {
            if (entityIn != entity && entityIn.riddenByEntity != entity && entityIn.ridingEntity != entity) {
                if (!entityCheck) {
                    AxisAlignedBB axisalignedbb = entity.getCollisionBoundingBox();

                    if (axisalignedbb != null && axisalignedbb.intersectsWith(boundingBox)) {
                        list.add(axisalignedbb);
                    }

                    AxisAlignedBB axisalignedbb2 = entityIn.getCollisionBox(entity);

                    if (axisalignedbb2 != null && axisalignedbb != axisalignedbb2 && axisalignedbb2.intersectsWith(boundingBox)) {
    ...
    public void updateEntities()
    {
        ...
        for (Entity entity : this.unloadedEntityList) {
            entity.deleteGLs();
            if (entity instanceof EntityPlayer player && player.nameTagsList != null) {
                player.nameTagsList.deleteLists();
    ...
    public Block checkBlockCollision2(AxisAlignedBB bb) {
        int i = MathHelper.floor_double(bb.minX);
        int j = MathHelper.floor_double(bb.maxX);
        int k = MathHelper.floor_double(bb.minY);
        int l = MathHelper.floor_double(bb.maxY);
    ...
    public List<Entity> getEntitiesInAABBexcluding(Entity entityIn, AxisAlignedBB boundingBox, Predicate<? super Entity > predicate) {
        ...
        int minX = MathHelper.floor_double((boundingBox.minX - 2.0D) / 16.0D);
        int maxX = MathHelper.floor_double((boundingBox.maxX + 2.0D) / 16.0D);
        int minZ = MathHelper.floor_double((boundingBox.minZ - 2.0D) / 16.0D);
        int maxZ = MathHelper.floor_double((boundingBox.maxZ + 2.0D) / 16.0D);
    ...
    public boolean canBlockBePlaced(Block blockIn, BlockPos pos, boolean p_175716_3_, EnumFacing side, Entity entityIn)
    {
        ...
        boolean nbr = Modules.PHASE.isEnabled() && Modules.PHASE.freePlace.getValue();
        return (nbr || (axisalignedbb == null || this.checkNoEntityCollision(axisalignedbb, entityIn))) && (block.getMaterial() == Material.circuits && blockIn == Blocks.anvil || block.getMaterial().isReplaceable() && blockIn.canReplace(this, pos, side));
    }
