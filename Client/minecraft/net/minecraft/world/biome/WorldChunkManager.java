...
public class WorldChunkManager
{
    ...
    protected WorldChunkManager()
    {
        ...
        this.biomesToSpawnIn.add(forest);
        this.biomesToSpawnIn.add(plains);
        this.biomesToSpawnIn.add(taiga);
        this.biomesToSpawnIn.add(taigaHills);
        this.biomesToSpawnIn.add(forestHills);
        this.biomesToSpawnIn.add(jungle);
        this.biomesToSpawnIn.add(jungleHills);
    }
    ...
    public float[] getRainfall(float[] listToReuse, int x, int z, int width, int length)
    {
        ...
        for (int i = 0; i < width * length; ++i)
        {
            try
            {
                float f = (float)BiomeGenBase.getBiomeFromBiomeList(aint[i], field_180279_ad).getIntRainfall() / 65536.0F;
    ...
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biomes, int x, int z, int width, int height)
    {
        ...
        try
        {
            ...
            {
                biomes[i] = BiomeGenBase.getBiomeFromBiomeList(aint[i], field_180279_ad);
            }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] listToReuse, int x, int z, int width, int length, boolean cacheFlag)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < width * length; ++i)
            {
                listToReuse[i] = BiomeGenBase.getBiomeFromBiomeList(aint[i], field_180279_ad);
            }
