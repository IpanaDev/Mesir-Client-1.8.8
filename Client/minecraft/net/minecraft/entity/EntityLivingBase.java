...
public abstract class EntityLivingBase extends Entity
{
    ...
    /**
     * DuraTest taken in the last hit. Mobs are resistant to damage less than this for a short time after taking damage.
     */
    ...
    private final int MAX_SPEEDS_LENGTH = 100;
    private List<Double> speeds = new ArrayList<>(MAX_SPEEDS_LENGTH);
    ...
    public void onEntityUpdate()
    {
        ...
        this.prevHurtResistantTime = this.hurtResistantTime;
    ...
    @Override
    public void onWatcherUpdateObjects(int id, Object lastObject, Object newObject) {
    ...
    public int getActivePotionLevel(Potion potionIn)
    {
        var potion = this.activePotionsMap.get(potionIn.id);
        return potion == null ? 0 : potion.getAmplifier() + 1;
    }
    /**
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (flag && s1 != null)
                    {
                        this.playSound(s1, this.getSoundVolume(), Math.max(this.getSoundPitch()-0.2f,0));
                    }
                }
    ...
    public void knockBack(double p_70653_3_, double p_70653_5_)
    {
        if (this.rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.knockbackResistance).getAttributeValue())
        {
            ...
            double f = MathHelper.sqrt_double(p_70653_3_ * p_70653_3_ + p_70653_5_ * p_70653_5_);
            float f1 = 0.4F;
            this.motionX /= 2.0D;
            this.motionY /= 2.0D;
            this.motionZ /= 2.0D;
            this.motionX -= p_70653_3_ / f * (double)f1;
            this.motionY += f1;
            this.motionZ -= p_70653_5_ / f * (double)f1;
    ...
    protected void damageEntity(DamageSource damageSrc, float damageAmount)
    {
        ...
        {
            //PlayerUtils.debug(damageAmount);
    ...
    public long lastGenericDamage;
    public long lastAttacked;

    public void handleStatusUpdate(byte id)
    {
        if (id == 2) {
            this.lastGenericDamage = System.currentTimeMillis();
            //this.limbSwingAmount = 1.5F;
            this.hurtResistantTime = this.maxHurtResistantTime;
            this.hurtTime = this.maxHurtTime = 10;
            this.attackedAtYaw = 0.0F;
            ...
            if (s != null)
            {
                //worldObj.playSound(this.posX, this.posY, this.posZ, this.getHurtSound(), this.getSoundVolume(), (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F + 1.0F, false);
    ...
    public void jump() {
        ...
        if (this instanceof EntityPlayerSP && BaritoneAPI.getProvider().getBaritoneForPlayer((EntityPlayerSP) this) != null) {
            yaw = this.jumpRotationEvent.getYaw();
        }
        if (this.isSprinting()) {
            if (this == Minecraft.getMinecraft().thePlayer && Modules.SPRINT.multi.getValue()) {
                this.motionX -= (MathHelper.sin(Minecraft.getMinecraft().thePlayer.getDirection(yaw)) * 0.2f);
                this.motionZ += (MathHelper.cos(Minecraft.getMinecraft().thePlayer.getDirection(yaw)) * 0.2f);
            } else {
                float f = yaw * 0.017453292F;
                float speed = 0.2f;
                this.motionX -= (MathHelper.sin(f) * speed);
                this.motionZ += (MathHelper.cos(f) * speed);
            }
        }
    ...
    {
        //PlayerUtils.debug(strafe+", "+forward);
        if (this.isServerWorld())
        {
            if ((!this.isInWater()) || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if ((!this.isInLava()) || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    ...
                    BaritoneHelper.travel(this, strafe, forward, f5);
                    f4 = 0.91F;
                    ...
                    double d1 = this.posY;
                    BaritoneHelper.travel(this, strafe, forward, 0.02F);
                    this.moveEntity(this.motionX, this.motionY, this.motionZ);
                    this.motionX *= 0.5D;
                    this.motionY *= 0.5D;
                    this.motionZ *= 0.5D;
            ...
            else
            {
                ...
                BaritoneHelper.travel(this, strafe, forward, f2);
                this.moveEntity(this.motionX, this.motionY, this.motionZ);
                this.motionX *= f1;
        ...
        double f7 = MathHelper.sqrt_double(d2 * d2 + d3 * d3) * 4.0F;
        ...
        this.limbSwingAmount += (float) ((f7 - this.limbSwingAmount) * 0.4F);
    ...
    public void onUpdate()
    {
        ...
        double speed = Math.hypot(d0, d1);
        speeds.add(speed);
        while (speeds.size() > MAX_SPEEDS_LENGTH) {
            speeds.removeFirst();
        }
    ...
    public double getAverageSpeed(int ticks) {
        ...
        for (int i = 0; i < ticks; i++) {
            int index = speeds.size() - 1 - i;
            if (index < 0) {
                break;
            }
            average += speeds.get(index);
        }
    ...
    public boolean canEntityBeSeen(double x, double y, double z) {
        return this.worldObj.rayTraceBlocks(new Vec3(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3(x, y, z)) == null;
    }
    public boolean canEntityBeSeenTranslucent(double x, double y, double z) {
        return this.worldObj.rayTraceBlocksTranslucent(new Vec3(this.posX, this.posY + (double)this.getEyeHeight(), this.posZ), new Vec3(x, y, z),false,true,false) == null;
    }
    /**
    ...
    public Vec3 getLook(float partialTicks)
    {
        ...
        else
        {
            ...
            return Modules.NUKER.isEnabled() ? this.getVectorForRotation(rotationPitchHead, rotationYawHead) : this.getVectorForRotation(rotationPitch, rotationYaw);
        }
    }
