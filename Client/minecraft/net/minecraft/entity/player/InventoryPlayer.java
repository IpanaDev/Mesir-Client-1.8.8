...
public class InventoryPlayer implements IInventory
{
    ...
    {
        if (Minecraft.getMinecraft().gameSettings.ofKeyBindZoom.pressed) {
            return;
        }
        if (p_70453_1_ > 0)
        {
    ...
    public int getSlotFor(ItemStack stack)
    {
        for (int i = 0; i < this.mainInventory.length; ++i)
        {
            if (mainInventory[i] != null && !this.mainInventory[i].func_190926_b() && this.stackEqualExact(stack, this.mainInventory[i]))
            {
                return i;
            }
        }
