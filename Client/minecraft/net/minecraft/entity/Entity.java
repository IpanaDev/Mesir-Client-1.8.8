...
import ipana.modules.movement.Step;
...
public abstract class Entity implements ICommandSender
{
     ...
     */
    public int prevHurtResistantTime;
    ...
    public GList<String>[] nameList = new GList[1];
    ...
    public Entity(World worldIn)
    {
        ...
        for (int i = 0; i < nameList.length; i++) {
            nameList[i] = new GList<>();
        }
        setRenderObject(Minecraft.getMinecraft().getRenderManager().renderEntity);
    }

    public void deleteGLs() {
        for (GList<String> gList : nameList) {
            gList.deleteList();
        }
    }
    ...
    public void onEntityUpdate()
    {
        ...
        this.worldObj.theProfiler.endSection();
        this.isInsideOfWater = isInsideOfMaterial(Material.water);
    }
    ...
    public void moveEntity(double x, double y, double z) {
        if (this.noClip) {
            ...
            AxisAlignedBB firstBB = this.getEntityBoundingBox();
            double d0 = this.posX;
            double d1 = this.posY;
            double d2 = this.posZ;
            ...
            double d4 = y;
            double d5 = z;
            Scaffold scaffold = Modules.SCAFFOLD;
            boolean scaffoldCheck = scaffold.isEnabled() && scaffold.mode.getValue() instanceof Blatant && scaffold.safeWalk.getValue();
            boolean anotherCheck = Modules.AUTO_TARGET.isEnabled() && Modules.AUTO_TARGET.safeWalk.getValue() && !Modules.KILL_AURA.targets.isEmpty();
            boolean flag = this instanceof EntityPlayer && (scaffoldCheck || (this.onGround && (this.isSneaking() || anotherCheck)));
            ...
            boolean hasX = x != 0;
            boolean hasZ = z != 0;
            boolean hasY = y != 0 && (y >= 0 || !onGround) || this == Minecraft.getMinecraft().getRenderViewEntity();
            if (hasX || hasY || hasZ) {
                List<AxisAlignedBB> boxes = this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().addCoord(x, y, z));
                if (hasY) {
                    for (AxisAlignedBB bb : boxes) {
                        y = bb.calculateYOffset(this.getEntityBoundingBox(), y);
                    }
                    this.setEntityBoundingBox(this.getEntityBoundingBox().offset(0.0D, y, 0.0D));
                }
                if (hasX) {
                    for (AxisAlignedBB bb : boxes) {
                        x = bb.calculateXOffset(this.getEntityBoundingBox(), x);
                    }
                ...
                if (hasZ) {
                    for (AxisAlignedBB bb : boxes) {
                        z = bb.calculateZOffset(this.getEntityBoundingBox(), z);
                    }
            ...
            Fly fly = Modules.FLY;
            boolean stepGround = this.onGround || (fly.isEnabled() && fly.mode.getValue() instanceof OldNCPGlide) && this == Minecraft.getMinecraft().thePlayer;
            boolean flag1 = stepGround || d4 != y && d4 < 0.0D;
            AxisAlignedBB collidedBB = this.getEntityBoundingBox();
            ...
            if (collidedBB != this.getEntityBoundingBox() && this == Minecraft.getMinecraft().getRenderViewEntity()) {
                Step step = Modules.STEP;
                if (step.isEnabled() && step.isPost()) {
                    EventStep event = new EventStep(collidedBB, this.getEntityBoundingBox());
                    event.fire();
                    if (event.isCancelled()) {
                        this.setEntityBoundingBox(firstBB);
                        this.worldObj.theProfiler.endSection();
                        return;
                    } else {
                        this.setEntityBoundingBox(collidedBB);
                    }
                }
            }
            this.worldObj.theProfiler.endSection();
            ...
            if (this.canTriggerWalking() && !flag && this.ridingEntity == null) {
                ...
                if (this.distanceWalkedOnStepModified > (float) this.nextStepDistance && block1.getMaterial() != Material.air) {
                    ...
                    if (this.isInWater()) {
                        double f = MathHelper.sqrt_double(this.motionX * this.motionX * 0.20000000298023224D + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.20000000298023224D) * 0.35F;

                        if (f > 1.0F) {
                            f = 1.0F;
                        }

                        this.playSound(this.getSwimSound(), (float) f, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
                    }
    ...
    {
        double f = MathHelper.sqrt_double(this.motionX * this.motionX * 0.20000000298023224D + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.20000000298023224D) * 0.2F;
        ...
        this.playSound(this.getSplashSound(), (float) f, 1.0F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.4F);
    ...
    public void setPositionAndRotationS08(double x, double y, double z, float yaw, float pitch) {
        this.posX = x;
        this.posY = y;
        this.posZ = z;
    ...
    public double getAverageHSpeed() {
    ...
    public float getHorizontalDistanceToEntity(Entity entityIn) {
    ...
    public double getDistance(BlockPos pos) {
        return MathHelper.sqrt_double(this.getDistanceSq(pos));
    }
    /**
    ...
    protected boolean isUpsideDown(String str) {
        String s = EnumChatFormatting.getTextWithoutFormattingCodes(str);
        return s != null && (s.equals("Dinnerbone") || s.equals("Grumm")) && (!(this instanceof EntityPlayer) || ((EntityPlayer)this).isWearing(EnumPlayerModelParts.CAPE));
    }
    ...
    public void setCustomNameTag(String name)
    {
        ...
        this.isDinnerbone = isUpsideDown(name);
    }
    ...
    public EnumFacing getHorizontalFacing() {
        return getHorizontalFacing(rotationYaw);
    }
    ...
    public void onWatcherUpdateObjects(int dataValueId, Object lastObject, Object newObject) {
