    ...
    public void updatePlayerList(List<EntityPlayer> p_73122_1_)
    {
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged())
        {
            if (this.trackedEntity.ridingEntity == null)
            {
                ...
                int x = MathHelper.floor_double(this.trackedEntity.posX * 32.0D);
                int y = MathHelper.floor_double(this.trackedEntity.posY * 32.0D);
                int z = MathHelper.floor_double(this.trackedEntity.posZ * 32.0D);
                int yaw = MathHelper.floor_float(this.trackedEntity.rotationYaw * 256.0F / 360.0F);
                int pitch = MathHelper.floor_float(this.trackedEntity.rotationPitch * 256.0F / 360.0F);
                int deltaX = x - this.encodedPosX;
                int deltaY = y - this.encodedPosY;
                int deltaZ = z - this.encodedPosZ;
                Packet packet1 = null;
                boolean flag = Math.abs(deltaX) >= 4 || Math.abs(deltaY) >= 4 || Math.abs(deltaZ) >= 4 || this.updateCounter % 60 == 0;
                boolean flag1 = Math.abs(yaw - this.encodedRotationYaw) >= 4 || Math.abs(pitch - this.encodedRotationPitch) >= 4;

                if (this.updateCounter > 0 || this.trackedEntity instanceof EntityArrow)
                {
                    if (deltaX >= -128 && deltaX < 128 && deltaY >= -128 && deltaY < 128 && deltaZ >= -128 && deltaZ < 128 && this.ticksSinceLastForcedTeleport <= 400 && !this.ridingEntity && this.onGround == this.trackedEntity.onGround)
                    {
                        if ((!flag || !flag1) && !(this.trackedEntity instanceof EntityArrow))
                        {
                            if (flag)
                            {
                                packet1 = new S14PacketEntity.S15PacketEntityRelMove(this.trackedEntity.getEntityId(), (byte)deltaX, (byte)deltaY, (byte)deltaZ, this.trackedEntity.onGround);
                            }
                            else if (flag1)
                            {
                                packet1 = new S14PacketEntity.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)yaw, (byte)pitch, this.trackedEntity.onGround);
                            }
                        }
                        else
                        {
                            packet1 = new S14PacketEntity.S17PacketEntityLookMove(this.trackedEntity.getEntityId(), (byte)deltaX, (byte)deltaY, (byte)deltaZ, (byte)yaw, (byte)pitch, this.trackedEntity.onGround);
                        }
                    }
                    else
                    {
                        ...
                        packet1 = new S18PacketEntityTeleport(this.trackedEntity.getEntityId(), x, y, z, (byte)yaw, (byte)pitch, this.trackedEntity.onGround);
                    }
                }
                ...
                if (flag1)
                {
                    this.encodedRotationYaw = yaw;
                    this.encodedRotationPitch = pitch;
                }
