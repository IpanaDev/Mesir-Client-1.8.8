...
public class ContainerEnchantment extends Container
{
    ...
    public void detectAndSendChanges()
    {
        ...
        for (ICrafting crafter : this.crafters) {
            ...
            crafter.sendProgressBarUpdate(this, 4, this.field_178151_h[0]);
            crafter.sendProgressBarUpdate(this, 5, this.field_178151_h[1]);
            crafter.sendProgressBarUpdate(this, 6, this.field_178151_h[2]);
        }
    }
    ...
    /**
     * Handles the given Cape-click on the server, currently only used by enchanting. Name is for legacy.
     */
    public boolean enchantItem(EntityPlayer playerIn, int id)
    {
        ...
        else if (this.enchantLevels[id] > 0 && itemstack != null && (playerIn.experienceLevel >= i && playerIn.experienceLevel >= this.enchantLevels[id] || playerIn.capabilities.isCreativeMode))
        {
            if (!this.worldPointer.isRemote)
            {
                ...
                for (EnchantmentData s : list) {
                    System.out.println(s.enchantmentobj.getName());
                }
