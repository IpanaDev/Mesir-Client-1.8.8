...
import ipana.utils.player.Flag;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void setWorld(WorldClient world) {
        this.clientWorldController = world;
    }
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn)
    {
        ...
        if (Minecraft.getMinecraft().isSingleplayer()) {
            Ipana.mainIRC().self().setAndSendProperty(UserProperties.SERVER, "SinglePlayer");
        } else if (Minecraft.getMinecraft().getCurrentServerData() != null) {
            Ipana.mainIRC().self().setAndSendProperty(UserProperties.SERVER, Minecraft.getMinecraft().getCurrentServerData().serverIP);
        }

        this.netManager.sendPacket(new C17PacketCustomPayload("MC|Brand", (new PacketBuffer(Unpooled.buffer())).writeString(ClientBrandRetriever.getClientModName())));
        //this.netManager.sendPacket(new C00PacketKeepAlive());
        if (Minecraft.getMinecraft().getCurrentServerData() != null) {
            if (Minecraft.getMinecraft().getCurrentServerData().serverIP.contains("seriex.software")) {
                System.out.println("Connecting to Seriex");
                this.netManager.sendPacket(new C17PacketCustomPayload("Seriex|2173", new PacketBuffer(Unpooled.buffer())));
            }
        }
    }
    ...
    public void handleSpawnObject(S0EPacketSpawnObject packetIn)
    {
        ...
        {
            int x = MathHelper.floor_double(d0);
            int y = MathHelper.floor_double(d1);
            int z = MathHelper.floor_double(d2);
            BlockPos blockPos = new BlockPos(x, y, z);
            entity = new EntityItemFrame(this.clientWorldController, blockPos, EnumFacing.getHorizontal(packetIn.func_149009_m()));
            ((EntityItemFrame) entity).updateChunk();
            packetIn.func_149002_g(0);
        }
    ...
    public void handleEntityMetadata(S1CPacketEntityMetadata packetIn) {
        ...
        if (entity != null && packetIn.watchableObjects() != null) {
            ...
            if (entity instanceof EntityItem entityItem) {
                for (DataWatcher.WatchableObject watchableObject : packetIn.watchableObjects()) {
                    if (watchableObject.getDataValueId() == 10) {
                        entityItem.setEntityItemStack((ItemStack) watchableObject.getObject());
                    }
                }
            }
             */
            entity.getDataWatcher().updateWatchedObjectsFromList(packetIn.watchableObjects());
        }
    }
    ...
    public void handleEntityMovement(S14PacketEntity packetIn)
    {
        ...
        if (entity != null)
        {
            ...
            entity.onGround = packetIn.getOnGround();
            entity.lastPosUpdateMS = System.currentTimeMillis();
        }
    }
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        EventSetBack pre = new EventSetBack(d0, d1, d2, f, f1, EventSetBack.State.PRE);
        for (int i = 0; i < PlayerUtils.predictedFlags.size(); i++) {
            Flag flag = PlayerUtils.predictedFlags.get(i);
            double offX = MathUtils.fixFormat(flag.x, 5);
            double offY = MathUtils.fixFormat(flag.y, 5);
            double offZ = MathUtils.fixFormat(flag.z, 5);
            double flagX = MathUtils.fixFormat(pre.xOff(), 5);
            double flagY = MathUtils.fixFormat(pre.yOff(), 5);
            double flagZ = MathUtils.fixFormat(pre.zOff(), 5);
            if (offX == flagX && offY == flagY && offZ == flagZ) {
                pre.cancelSetPos(true);
                pre.cancelPacket(true);
                PlayerUtils.predictedFlags.remove(i);
                pre.setResetMotion(false);
                break;
            }
        }
        pre.fire();
        ...
        if (Modules.LESS_PACKETS.isEnabled()) {
            if (!pre.cancelSetPos()) {
                entityplayer.setPositionAndRotationS08(d0, d1, d2, f, f1);
            }
            if (!pre.cancelPacket()) {
                this.gameController.thePlayer.packetQueue.add(new LessPackets.PlayerPosLook(d0, d1, d2, f, f1));
            }
        } else {
            if (!pre.cancelSetPos()) {
            ...
            if (!pre.cancelPacket()) {
                if (!pre.cancelSetPos()) {
                    //PlayerUtils.debug("flag: "+packetIn.getY());
                    //PlayerUtils.debug("real: ",entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, entityplayer.rotationYaw, entityplayer.rotationPitch);
                    ...
                    //entityplayer.report(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, entityplayer.rotationYaw, entityplayer.rotationPitch);
                } else {
                    this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(d0, d1, d2, f, f1, false));
                    //entityplayer.report(d0, d1, d2, f, f1);
                }
            }
        }
        if (Modules.NO_ROTATE.isEnabled() && Modules.NO_ROTATE.silent.getValue()) {
            entityplayer.setRotation(yaw, pitch);
            entityplayer.prevRotationYaw = yaw % 360;
            entityplayer.prevRotationPitch = pitch % 360;


            entityplayer.report(f, f1);
        }
        EventSetBack post = new EventSetBack(d0, d1, d2, f, f1, EventSetBack.State.POST);
        post.cancelPacket(pre.cancelPacket());
        post.cancelSetPos(pre.cancelSetPos());
        post.fire();
        PlayerUtils.predictedFlags.removeIf(flag -> System.currentTimeMillis()-flag.ms >= 10000);
    ...
    public void handleMultiBlockChange(S22PacketMultiBlockChange packetIn)
    {
        ...
        {
            //PlayerUtils.debug("MultiBlockChange: "+(data.getPos())+", "+(data.getBlockState()));
            this.clientWorldController.invalidateRegionAndSetBlock(data.getPos(), data.getBlockState());
        }
        ...
        ChunkCoordIntPair pos = new ChunkCoordIntPair(packetIn.getChangedBlocks()[0].getPos());
        for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
            EntityPlayerSP player = ibaritone.getPlayerContext().player();
            if (player != null && player.sendQueue == this) {
                ibaritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL, pos.chunkXPos, pos.chunkZPos));
            }
        }
    }
    ...
    public void handleChunkData(S21PacketChunkData packetIn)
    {
        ...
        for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
            EntityPlayerSP player = ibaritone.getPlayerContext().player();
            if (player != null && player.sendQueue == this) {
                ibaritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.PRE, packetIn.isFullChunk() ? ChunkEvent.Type.POPULATE_FULL : ChunkEvent.Type.POPULATE_PARTIAL, packetIn.getChunkX(), packetIn.getChunkZ()));
            }
        }
        chunk.fillChunk(packetIn.func_149272_d(), packetIn.getExtractedSize(), packetIn.isFullChunk());
        ...
        for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
            EntityPlayerSP player = ibaritone.getPlayerContext().player();
            if (player != null && player.sendQueue == this) {
                ibaritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, packetIn.isFullChunk() ? ChunkEvent.Type.POPULATE_FULL : ChunkEvent.Type.POPULATE_PARTIAL, packetIn.getChunkX(), packetIn.getChunkZ()));
            }
        }
    }
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        //PlayerUtils.debug("BlockChange: "+packetIn.getBlockPosition()+", "+packetIn.getBlockState());
        ...
        for (IBaritone ibaritone : BaritoneAPI.getProvider().getAllBaritones()) {
            EntityPlayerSP player = ibaritone.getPlayerContext().player();
            if (player != null && player.sendQueue == this) {
                ibaritone.getGameEventHandler().onChunkEvent(new ChunkEvent(EventState.POST, ChunkEvent.Type.POPULATE_FULL, packetIn.getBlockPosition().getX() >> 4, packetIn.getBlockPosition().getZ() >> 4));
            }
        }
    }
    ...
    public void handleChat(S02PacketChat packetIn)
    {
        ...
        if (packetIn.getChatComponent().getFormattedText().contains("Destroying your own vehicle is disabled.")) {
            return;
        }
        ...
        else
        {
            ...
            this.gameController.ingameGUI.getChatGUI().messagesUnseen++;
        }
    }
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        this.gameController.thePlayer.setSpawnPoint(packetIn.getSpawnPos().add(0,10,0), true);
        this.gameController.theWorld.getWorldInfo().setSpawn(packetIn.getSpawnPos().add(0,10,0));
    }
    ...
    public void handleSetSlot(S2FPacketSetSlot packetIn) {
        ...
        if (packetIn.windowId() == -1) {
            entityplayer.inventory.setItemStack(packetIn.itemStack());
        } else {
            boolean flag = false;
            ...
            if (packetIn.windowId() == 0 && packetIn.slot() >= 36 && packetIn.slot() < 45) {
                ItemStack itemstack = entityplayer.inventoryContainer.getSlot(packetIn.slot()).getStack();

                if (packetIn.itemStack() != null && (itemstack == null || itemstack.stackSize < packetIn.itemStack().stackSize)) {
                    packetIn.itemStack().animationsToGo = 5;
                }
                entityplayer.inventoryContainer.putStackInSlot(packetIn.slot(), packetIn.itemStack());
            } else if (packetIn.windowId() == entityplayer.openContainer.windowId && (packetIn.windowId() != 0 || !flag)) {
                entityplayer.openContainer.putStackInSlot(packetIn.slot(), packetIn.itemStack());
            }
        }
    }
    ...
    public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn)
    {
        ...
        if (container != null && !packetIn.accepted())
        {
            //PlayerUtils.debug("SEND");
        ...
        if (this.gameController.theWorld.isBlockLoaded(packetIn.getPos()))
        {
            ...
            if (tileentity instanceof TileEntitySign tileentitysign)
            {
                ...
                if (tileentitysign.getIsEditable())
                {
                    ...
                    tileentitysign.needsUpdate = true;
    ...
    public void handleCombatEvent(S42PacketCombatEvent packetIn)
    {
        ...
        //PlayerUtils.debug(packetIn.eventType);
