...
public class MapItemRenderer
{
    ...
    public class Instance
    {
        ...
        private void render(boolean noOverlayRendering) {
            ...
            worldrenderer.pos(((float)(i + 128) - f), ((float)(0) + f), -0.009999999776482582D).tex(1.0D, 0.0D).endVertex();
            worldrenderer.pos(((float)(0) + f), ((float)(0) + f), -0.009999999776482582D).tex(0.0D, 0.0D).endVertex();
            tessellator.draw();
            ...
            for (Vec4b vec4b : this.mapData.mapDecorations.values())
            {
                if (!noOverlayRendering || vec4b.func_176110_a() == 1)
                {
                    ...
                    worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
                    float f5 = -0.001F;
                    worldrenderer.pos(-1.0D, 1.0D, ((float)k * -0.001F)).tex(f1, (double)f2).endVertex();
                    worldrenderer.pos(1.0D, 1.0D, ((float)k * -0.001F)).tex(f3, (double)f2).endVertex();
                    worldrenderer.pos(1.0D, -1.0D, ((float)k * -0.001F)).tex(f3, (double)f4).endVertex();
                    worldrenderer.pos(-1.0D, -1.0D, ((float)k * -0.001F)).tex(f1, (double)f4).endVertex();
                    tessellator.draw();
