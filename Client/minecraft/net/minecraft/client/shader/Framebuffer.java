...
public class Framebuffer {
    ...
    private float lastRenderX, lastRenderY;
    private GList<Object> gList = new GList<>();
    /*
    private VList<Object> vList = new VList<>(GL11.GL_QUADS, DefaultVertexFormats.POSITION_TEX_COLOR, c -> {
        int stride = 24;
    ...
    public Framebuffer(int p_i45078_1_, int p_i45078_2_, boolean p_i45078_3_, Object o) {
        ...
        this.createBindFramebuffer(p_i45078_1_, p_i45078_2_, o);
    }

    public void createBindFramebuffer(int width, int height, Object o) {
        if (!OpenGlHelper.isFramebufferEnabled()) {
            ...
            this.createFramebuffer(width, height, o);
    ...
    public void deleteFramebuffer() {
        if (OpenGlHelper.isFramebufferEnabled()) {
            //vList.deleteList();
            gList.deleteList();
            lastWidth = lastHeight = lastRenderX = lastRenderY = 0;
    ...
    public void createFramebuffer(int width, int height, Object o) {
        float scaleRatio = Minecraft.getMinecraft().gameSettings.renderScale / 100f;
        this.scaledWidth = (int) (width * scaleRatio);
        this.scaledHeight = (int) (height * scaleRatio);
        this.framebufferWidth = scaledWidth;
        this.framebufferHeight = scaledHeight;
        this.framebufferTextureWidth = scaledWidth;
        this.framebufferTextureHeight = scaledHeight;
    ...
    public void framebufferRenderExt(int p_178038_1_, int p_178038_2_, boolean p_178038_3_) {
        if (OpenGlHelper.isFramebufferEnabled()) {
            ...
            GlStateManager.ortho(0, p_178038_1_, p_178038_2_, 0.0D, 1000.0D, 3000.0D);
            ...
            if (lastWidth != f2 || lastHeight != f3 || lastRenderX != f || lastRenderY != f1) {
                gList.compile(c -> {
                    Tessellator tessellator = Tessellator.getInstance();
                    WorldRenderer worldrenderer = tessellator.getWorldRenderer();
                    worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                    worldrenderer.pos(0, f1, 0).tex(0, 0).color(255, 255, 255, 255).endVertex();
                    worldrenderer.pos(f, f1, 0).tex(f2, 0).color(255, 255, 255, 255).endVertex();
                    worldrenderer.pos(f, 0, 0).tex(f2, f3).color(255, 255, 255, 255).endVertex();
                    worldrenderer.pos(0, 0, 0).tex(0, f3).color(255, 255, 255, 255).endVertex();
                    tessellator.draw();
                });
