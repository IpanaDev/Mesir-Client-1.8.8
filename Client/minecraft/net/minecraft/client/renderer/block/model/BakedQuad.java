...
public class BakedQuad implements IVertexProducer
{
    ...
    public int[] getOffsetVertexData(int xValue, int yValue, int zValue) {
        int[] v = getVertexData().clone();
        if (xValue != 0 || yValue != 0 || zValue != 0) {
            for (int i = 0; i < v.length; i += 7) {
                float x = Float.intBitsToFloat(v[i])   * xValue;
                float y = Float.intBitsToFloat(v[i+1]) * yValue;
                float z = Float.intBitsToFloat(v[i+2]) * zValue;
                v[i]     = Float.floatToRawIntBits(x);
                v[i + 1] = Float.floatToRawIntBits(y);
                v[i + 2] = Float.floatToRawIntBits(z);
            }
        }
        return v;
    }
