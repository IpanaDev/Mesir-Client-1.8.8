...
public abstract class Render<T extends Entity>
{
    ...
    {
        if (Minecraft.getRunTick() != livingEntity.renderTicks) {
            ...
            livingEntity.renderChecks = livingEntity.isInRangeToRender3d(camX, camY, camZ) && (livingEntity.ignoreFrustumCheck || camera.isBoundingBoxInFrustum(axisalignedbb));
            livingEntity.renderTicks = Minecraft.getRunTick();
        }
        return livingEntity.renderChecks;
    }
    ...
    public static void renderOffsetAABB(AxisAlignedBB boundingBox, double x, double y, double z)
    {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        worldrenderer.setTranslation((float) x, (float)y, (float)z);
    ...
    public void doRenderShadowAndFire(Entity entityIn, double x, double y, double z, float partialTicks)
    {
        ...
        {
            if (this.renderManager.options.isEntityShadows && this.shadowSize > 0.0F && !entityIn.isInvisible() && this.renderManager.isRenderShadow())
            {
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z) {
        ...
        str = StringUtil.preventCrash(str);
        if (d0 <= (double) (64 * 64)) {
            String finalStr = str;
            GlStateManager.pushMatrix();
            ...
            if (camera.isEnabled()) {
                ...
                GlStateManager.rotate(camera.pitch, 1.0F, 0.0F, 0.0F);
            } else {
                GlStateManager.rotate(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
            ...
            GLCall.draw(entityIn.nameList[0], str, c -> {
                ...
                if (finalStr.equals("deadmau5")) {
                    b0 = -10;
                }

                int i = fontrenderer.getStringWidth(finalStr) / 2;
                GL11.glDisable(GL11.GL_TEXTURE_2D);
                byte finalB = b0;

                worldrenderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
                worldrenderer.pos((-i - 1), (-1 + finalB), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).normal(0.0F, 1.0F, 0.0F).endVertex();
                worldrenderer.pos((-i - 1), (8 + finalB), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).normal(0.0F, 1.0F, 0.0F).endVertex();
                worldrenderer.pos((i + 1), (8 + finalB), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).normal(0.0F, 1.0F, 0.0F).endVertex();
                worldrenderer.pos((i + 1), (-1 + finalB), 0.0D).color(0.0F, 0.0F, 0.0F, 0.25F).normal(0.0F, 1.0F, 0.0F).endVertex();
                tessellator.draw();
                ...
                fontrenderer.drawString(finalStr, -fontrenderer.getStringWidth(finalStr) / 2f, finalB, 553648127);
                GL11.glEnable(GL11.GL_DEPTH_TEST);
                GL11.glDepthMask(true);
                fontrenderer.drawString(finalStr, -fontrenderer.getStringWidth(finalStr) / 2f, finalB, -1);
