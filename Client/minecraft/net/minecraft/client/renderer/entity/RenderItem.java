...
import net.minecraft.client.resources.model.BakedData;
...
public class RenderItem implements IResourceManagerReloadListener {
    ...
    private void renderModel(IBakedModel model, int color, ItemStack stack) {
        if (Config.isGlCalls()) {
            boolean isMetadataRequired = stack != null && (stack.getItem() instanceof ItemPotion || stack.getItem() instanceof ItemMonsterPlacer);
            for (BakedData data : model.bakedLists()) {
                if (data.model() == model && data.color() == color && (!isMetadataRequired || data.metadata() == stack.getItem().getColorFromItemStack(stack, 0))) {
                    data.gList().render();
                    return;
                }
            }
            GList<IBakedModel> bakedList = new GList<>();
            bakedList.render(model, c -> renderModel0(model, color, stack, null));
            model.bakedLists().add(new BakedData(model, isMetadataRequired ? stack.getItem().getColorFromItemStack(stack, 0) : 0, color, bakedList));
        } else {
            renderModel0(model, color, stack, null);
        }
    }

    private void renderModel0(IBakedModel model, int color, ItemStack stack, VList<IBakedModel> vList) {
        ...
        if (vList == null) {
            ...
            worldRenderer.reset();
            vList.compile(worldRenderer.getByteBuffer());
        }
    ...
    public void renderItem(ItemStack stack, IBakedModel model) {
        if (stack != null) {
            ...
            if (model.isBuiltInRenderer()) {
                ...
                GlStateManager.bindTexture(TextureMap.locationBlocksTexture.textureId.getGlTextureId());
        ...
        if (stack != null && entityToRenderFor != null) {
            ...
            if (entityToRenderFor instanceof EntityPlayer entityplayer) {
                ...
                if (item == Items.fishing_rod && entityplayer.fishEntity != null) {
                    ...
                    if (entityplayer.getHeldItem() != null && entityplayer.getHeldItem().getItem() != null && (entityplayer.getItemInUse() != null)) {
                        if ((entityplayer.getItemInUseCount() > 0)) {
                            if (entityplayer.getHeldItem().getItemUseAction() == EnumAction.BLOCK) {
                                if (Modules.BLOCK_ANIM.mode.getValue() == BlockAnim.Mode.OLD) {
                                    GL11.glTranslatef(-0.05F, 0.05F, -0.1F);
                                } else if (Modules.BLOCK_ANIM.mode.getValue() == BlockAnim.Mode.CUSTOM) {
                                    GL11.glTranslatef(0.0F, 0.0F, 0.05F);
                                }
                                if (Modules.BLOCK_ANIM.mode.getValue() == BlockAnim.Mode.CUSTOM) {
                                    GL11.glRotatef(30.0F, 1.0F, 0.0F, 0.0F);
                                    GL11.glRotatef(-90.0F, 1.0F, 0.0F, 0.0F);
                                    GL11.glRotatef(90.0F, 0.0F, 0.0F, 1.0F);
                                }
                                if (Modules.BLOCK_ANIM.mode.getValue() != BlockAnim.Mode.NEW) {
                    ...
                    if (entityplayer.getHeldItem() != null && entityplayer.getHeldItem().getItem() != null && entityplayer.getItemInUse() != null) {
                        if ((entityplayer.getItemInUseCount() > 0)) {
                            if (entityplayer.getHeldItem().getItemUseAction() == EnumAction.DRINK || entityplayer.getHeldItem().getItemUseAction() == EnumAction.EAT) {
            ...
            PlayerAnim anim = Anims.getAnimByName(entityToRenderFor,":nah:");
            boolean cancel = anim != null;
            if (!cancel)
    ...
    public void renderItemIntoGUI(ItemStack stack, int x, int y, boolean depth) {
        this.renderItemGui = true;
        IBakedModel ibakedmodel = this.itemModelMesher.getItemModel(stack);
        GlStateManager.pushMatrix();
        this.textureManager.bindTexture(TextureMap.locationBlocksTexture);
    ...
    public void renderItemAndEffectIntoGUI(final ItemStack stack, int xPosition, int yPosition, boolean depth) {
        if (stack != null && stack.getItem() != null) {
            this.zLevel += 50.0F;

            try {
                this.renderItemIntoGUI(stack, xPosition, yPosition, depth);
            } catch (Throwable throwable) {
                CrashReport crashreport = CrashReport.makeCrashReport(throwable, "Rendering item");
                CrashReportCategory crashreportcategory = crashreport.makeCategory("Item being rendered");
                crashreportcategory.addCrashSectionCallable("Item Type", () -> String.valueOf(stack.getItem()));
