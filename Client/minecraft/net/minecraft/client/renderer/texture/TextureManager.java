...
import ipana.utils.render.ThreadDownloadTextureImage;
...
public class TextureManager implements ITickable, IResourceManagerReloadListener
{
    ...
    public void bindTexture(ResourceLocation resource) {
        this.bindTexture(resource, false);
    }

    public void bindTexture(ResourceLocation resource, boolean force) {
        if (resource.textureId == null) {
            bindTexture0(resource);
        } else {
            if (Config.isShaders()) {
                ShadersTex.bindTexture(resource.textureId);
            } else {
                if (force) {
                    GlStateManager.forceBindTexture(resource.textureId.getGlTextureId());
                } else {
                    GlStateManager.bindTexture(resource.textureId.getGlTextureId());
                }
            }
        }
    }
    private void bindTexture0(ResourceLocation resource) {
    ...
    public boolean loadTexture(ResourceLocation textureLocation, ITextureObject textureObj) {
        ...
        try {
            ...
            textureLocation.textureId = textureObj;
            ...
            crashreportcategory.addCrashSectionCallable("Texture object class", (Callable) () -> textureObj.getClass().getName());
            throw new ReportedException(crashreport);
        }
        if (textureLocation == TextureMap.locationBlocksTexture) {
            TextureMap.locationBlocksTextureObject = itextureobject;
        }
        this.mapTextureObjects.put(textureLocation, itextureobject);
        return flag;
    }
    ...
    public void deleteTexture(ResourceLocation textureLocation)
    {
        ...
        if (itextureobject != null)
        {
            textureLocation.textureId = null;
