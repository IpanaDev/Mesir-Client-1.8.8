...
public class ChunkRenderWorker implements Runnable {
    ...
    protected void processTask(final ChunkCompileTaskGenerator generator) throws InterruptedException {
        ...
        if (viewEntity == null) {
            ...
            float f = (float) viewEntity.posX;
            float f1 = (float) viewEntity.posY + viewEntity.getEyeHeight();
            float f2 = (float) viewEntity.posZ;
            ...
            ArrayList<ListenableFuture<Object>> uploadedChunks = Lists.newArrayList();

            if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.REBUILD_CHUNK) {
                for (EnumWorldBlockLayer enumworldblocklayer : EnumWorldBlockLayer.VALUES) {
                    if (compiledChunk.isLayerStarted(enumworldblocklayer)) {
                        uploadedChunks.add(this.chunkRenderDispatcher.uploadChunk(enumworldblocklayer, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(enumworldblocklayer), generator.getRenderChunk(), compiledChunk));
                    }
                }
            } else if (chunkcompiletaskgenerator$type == ChunkCompileTaskGenerator.Type.RESORT_TRANSPARENCY) {
                uploadedChunks.add(this.chunkRenderDispatcher.uploadChunk(EnumWorldBlockLayer.TRANSLUCENT, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(EnumWorldBlockLayer.TRANSLUCENT), generator.getRenderChunk(), compiledChunk));
            }
            final ListenableFuture<List<Object>> listenablefuture = Futures.allAsList(uploadedChunks);
    ...
    public RegionRenderCacheBuilder getRegionRenderCacheBuilder() throws InterruptedException {
